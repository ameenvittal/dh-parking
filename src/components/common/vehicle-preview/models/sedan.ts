...
export function createSedan(paintColor: string, plateTexture: THREE.CanvasTexture, isEV = false): THREE.Group {
  ...
  // Chassis / Lower Body (Longer Sedan lower)
  const lowerGeo = new THREE.BoxGeometry(1.42, 0.52, 2.95)
  const lower = new THREE.Mesh(lowerGeo, paintMat)
  lower.position.set(0, 0.46, 0)
  group.add(lower)

  // Cabin / Upper Body (Sleek sedan roof profile)
  const cabinGeo = new THREE.BoxGeometry(1.28, 0.5, 1.4)
  const cabin = new THREE.Mesh(cabinGeo, paintMat)
  cabin.position.set(0, 0.92, -0.05)
  group.add(cabin)

  // Sloped Windows (Front and Rear windscreen slope)
  const glassFrontGeo = new THREE.BoxGeometry(1.22, 0.42, 0.45)
  const glassFront = new THREE.Mesh(glassFrontGeo, glassMat)
  glassFront.position.set(0, 0.93, 0.5)
  group.add(glassFront)

  const glassRearGeo = new THREE.BoxGeometry(1.22, 0.42, 0.45)
  const glassRear = new THREE.Mesh(glassRearGeo, glassMat)
  glassRear.position.set(0, 0.93, -0.6)
  group.add(glassRear)
  ...
  if (isEV) {
    ...
    port.position.set(0.72, 0.6, 0.6)
    group.add(port)
  }
